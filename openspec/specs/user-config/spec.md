...
The single local user (name, phone) is modeled as configuration instead of the
...
The system MUST resolve the active user from `USER_NAME` and `USER_PHONE`
validated by Zod, and MUST expose it to services as an immutable value.

#### Scenario: User config loads

- GIVEN `USER_NAME`, `USER_PHONE` are set
...
The system MUST derive the user folder from the local config (`{USER_NAME}`)
and MUST keep the WhatsApp last-message behavior.
